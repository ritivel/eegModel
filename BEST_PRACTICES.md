# Best practices — small-scale → large-scale EEG pretraining methodology
...
> should be edited as the project progresses.
>
> This file is the durable engineering / research discipline for the repo.
> Per-experiment narrative, results, and decisions live in Notion (the
> EEG Foundation Models — Operations Hub). This file is what an agent
> reads before designing or reviewing an experiment.
...
writing any trainer. The audit format we use:
...
This is exactly what saved a prior CTC-decoding experiment: the audit
found **19 hours of EEG silently chopped off**, **NaN noise in the
matched-control twin**, **channel-mismatch zero-padding > 80% of rows**,
and **8 garbage labels** — all data bugs that would have invalidated
weeks of pretraining if not caught upfront. Hold any future pretraining
run to the same standard.
...
| **Linear probe regression on HBN externalizing-factor + attention-factor** (primary, R² + MAE)                                                          | Continuous label = richer signal than binary AUROC; matches NeurIPS 2025 EEG Foundation Challenge C2 directly. HBN ships no DSM-V Dx columns; CBCL Pearson-z factors are the canonical eval target | NeurIPS 2025 EEG-FM challenge winners (e.g. ST-EEGFormer 0.88668)                                                            |
| **Attention-binary AUROC at z>+0.5σ** (secondary, for binary-AUROC continuity)                                                                          | ~28% positive rate, similar class balance to TUAB normal/abnormal                       | A binary read-out of the same label as the regression task above                                                              |
...
- **Columns = the controls** (often: target model size × { with intervention, without intervention } and ideally a "noise" control matched to your study, the matched-noise twin pattern).

Concretely, the canonical matrix shape for SSL pretraining ablations:
...
channel-naming convention) and use that as a shortcut. The classical
defense is *aggressive* canonical offline preprocessing (resample +
notch + bandpass + per-recording z-score + 15-σ clip). For SSL
...
z-score + ±5σ clip + 4-second windowing per the per-experiment
pretraining-corpus spec in Notion),
