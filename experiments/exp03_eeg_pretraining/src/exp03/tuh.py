...
``exp03 tuh-rsync``), which mirrors a local copy under
``$EXP03_DATA_ROOT/raw/{tuab,tuev}/``. This module then walks that local
copy.
...
def _walk_tuab(corpus_root: Path, version: str) -> list[Recording]:
    ...
    if not edf_root.exists():
        raise FileNotFoundError(
            ...
            f"Try `exp03 tuh-rsync tuab` first."
        )
...
def _walk_tuev(corpus_root: Path, version: str) -> list[Recording]:
    ...
    if not edf_root.exists():
        raise FileNotFoundError(
            ...
            f"Try `exp03 tuh-rsync tuev` first."
        )
...
def rsync_corpus(corpus: TUH_CORPUS_T, local_root: Path,
    ...
    ``local_root`` is the per-corpus root, e.g. ``$EXP03_DATA_ROOT/raw/tuab/``.
