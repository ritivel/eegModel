...
#
# Usage:
#   # On the GPU box, with the exp03 venv active and SSH agent
...
#   EXP03_DATA_ROOT   — default /opt/dlami/nvme/eeg
...
# setup
# -------------------------------------------------------------------
: "${EXP03_DATA_ROOT:=/opt/dlami/nvme/eeg}"
export EXP03_DATA_ROOT

SCRATCH="${EXP03_DATA_ROOT}/scratch"
mkdir -p "$SCRATCH"

REPO_ROOT="${REPO_ROOT:-/home/ubuntu/eegModel}"
RESULTS_MD="${REPO_ROOT}/experiments/exp03_eeg_pretraining/mini_experiments/01_sanity_baselines/results.md"

# Activate the exp03 venv so `exp03` is on PATH
...
RUN_DIR="${EXP03_DATA_ROOT}/runs/01_sanity_baselines/${RUN_TS}_track_a"
mkdir -p "$RUN_DIR"
...
log "PID $$"
log "EXP03_DATA_ROOT=$EXP03_DATA_ROOT"
log "RUN_DIR=$RUN_DIR"
log "==============================================================="
...
if [[ "${TRACK_A_SKIP_RSYNC:-0}" == "1" ]]; then
  ...
  if ! exp03 tuh-rsync tuab >>"$LOG" 2>&1; then
    log "ERROR: TUAB rsync failed; see $LOG"
    exit 1
  fi
  log "step 1: rsync TUEV v2.0.1"
  if ! exp03 tuh-rsync tuev >>"$LOG" 2>&1; then
...
if [[ "${TRACK_A_SKIP_PREPROCESS:-0}" == "1" ]]; then
  ...
  if ! exp03 tuh-preprocess tuab --pipeline v2_clean "${PRE_FLAGS[@]}" >>"$LOG" 2>&1; then
  ...
  if ! exp03 tuh-preprocess tuev --pipeline v2_clean "${PRE_FLAGS[@]}" >>"$LOG" 2>&1; then
...
if [[ "${TRACK_A_SKIP_SYNC:-0}" == "1" ]]; then
  ...
  if ! exp03 sync-derived-up --pipeline tuh >>"$LOG" 2>&1; then
...
HBN_DERIVED="${EXP03_DATA_ROOT}/derived/hbn_minimal_500hz"
TUAB_DERIVED="${EXP03_DATA_ROOT}/derived/tuab_v2_clean_250hz"
TUEV_DERIVED="${EXP03_DATA_ROOT}/derived/tuev_v2_clean_250hz"

if [[ ! -d "$HBN_DERIVED" ]]; then
  log "step 4: HBN parquet not on NVMe; pulling from S3 (small subset for the probe call)..."
  if ! exp03 sync-derived-down --pipeline minimal >>"$LOG" 2>&1; then
...
if ! python -m exp03.sanity check-d \
...
# -------------------------------------------------------------------
if [[ "${TRACK_A_SKIP_SYNC:-0}" != "1" ]]; then
  log "step 7: sync $RUN_DIR → s3://eegmodel-warehouse/runs/exp03/01_sanity_baselines/${RUN_TS}_track_a/"
  aws s3 cp --recursive "$RUN_DIR" \
    "s3://eegmodel-warehouse/runs/exp03/01_sanity_baselines/${RUN_TS}_track_a/" \
