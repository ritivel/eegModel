"""eegfm CLI — entrypoints for everything we run from the GPU box.

Top-level commands (each a ``typer`` subcommand):

    eegfm paths                  — print the resolved storage layout (sanity)
    eegfm list-releases          — list all HBN-EEG releases on s3://fcp-indi/
    eegfm list-subjects          — list subjects in one release
    eegfm download               — pull raw .set/.fdt + sidecars for a (release, subject) range
    eegfm audit                  — Phase-0 data audit (Karpathy step 1) on the local raw dir
    eegfm preprocess             — apply minimal (and/or v2_clean) pipelines → parquet shards (HBN)
    eegfm tuh-rsync              — rsync TUAB / TUEV from NEDC SFTP into local NVMe
    eegfm tuh-preprocess         — apply v2_clean pipeline → parquet shards (TUH)
    eegfm train                  — run one SSL pretraining cell (any paradigm) with wandb logging
    eegfm sync-derived-up        — rclone the local derived/ tree to s3://eegmodel-warehouse/
    eegfm sync-derived-down      — rclone s3://eegmodel-warehouse/derived/ → local NVMe (bootstrap)

All commands respect ``$EEG_DATA_ROOT`` (default ``/opt/dlami/nvme/eeg``).
"""
...
app = typer.Typer(
    ...
    help=(
        "eegfm — self-supervised EEG pretraining on HBN-EEG. "
        "All paths and S3 prefixes resolve from $EEG_DATA_ROOT (default "
...
# ---------------------------------------------------------------------------
# `eegfm paths` — sanity check the resolved storage layout
# ---------------------------------------------------------------------------
...
def paths_cmd():
    ...
    s = storage.from_env()
    table = Table(title="eegfm storage layout", show_lines=False)
    ...
    table.add_row("[s3] runs/eegfm", s.s3_uri("runs", "eegfm"))
    console.print(table)


# ---------------------------------------------------------------------------
# `eegfm list-releases` and `eegfm list-subjects`
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# `eegfm audit` — Phase-0 data audit (per methodology.md §2 Phase 0)
# ---------------------------------------------------------------------------
...
def audit_cmd(
    ...
    if not rel_dir.exists():
        console.print(f"[red]error:[/red] {rel_dir} not found — run "
                      f"`eegfm download {release}` first")
        raise typer.Exit(1)
...
def preprocess_cmd(
    ...
    under ``$EEG_DATA_ROOT/derived/<pipeline>/sub-<id>/``.
    ...
    if not rel_dir.exists():
        console.print(f"[red]error:[/red] no local data for release {release} at {rel_dir} — "
                      f"run `eegfm download {release}` first")
        raise typer.Exit(1)
...
# ---------------------------------------------------------------------------
# `eegfm tuh-rsync` — pull a TUH corpus from NEDC SFTP
# ---------------------------------------------------------------------------
...
def tuh_rsync_cmd(
    ...
    """rsync a TUH corpus from NEDC SFTP into ``$EEG_DATA_ROOT/raw/<corpus>/``.
    ...
    if rc != 0:
        ...
        raise typer.Exit(rc)
    console.print(f"[green]done.[/green] Use `eegfm tuh-preprocess {corpus}` next.")


# ---------------------------------------------------------------------------
# `eegfm tuh-preprocess` — build TUH parquet shards (Protocol A.4 fuel)
# ---------------------------------------------------------------------------
...
def tuh_preprocess_cmd(
    ...
    if not recordings:
        console.print(f"[red]error:[/red] no recordings under {corpus_root} for splits {splits_t}; "
                      f"did `eegfm tuh-rsync {corpus}` complete?")
        raise typer.Exit(1)
...
# ---------------------------------------------------------------------------
# `eegfm train` — run one SSL pretraining cell (paradigm × seed)
# ---------------------------------------------------------------------------
...
def train_cmd(
    ...
    data_root: Path = typer.Option(None, "--data-root",
                                   help="Override $EEG_DATA_ROOT/derived/<pipeline>/."),
    ...
    output_dir: Path = typer.Option(None, "--output-dir",
                                    help="Where to write checkpoints + summary.json. Default: "
                                         "$EEG_DATA_ROOT/runs/<wandb_run_name or auto>."),
    ...
    \b
        ...
        eegfm train --paradigm mae --steps 100 --wandb-mode disabled --no-eval-at-end

    \b
        # Real exp17 cell: G2 MAR, ~35M tokens (= 17500 steps × batch 32 × patch 8 / 2000 samples)
        eegfm train --paradigm mar --steps 17500 --batch-size 32 \\
...
# ---------------------------------------------------------------------------
# `eegfm sync-derived-up` / `eegfm sync-derived-down`
# ---------------------------------------------------------------------------
...
def sync_up_cmd(
    ...
    Idempotent (rclone copy). Use after `eegfm preprocess` or
    `eegfm tuh-preprocess` to materialise the warehouse mirror.
    """
    import subprocess
...
def main(argv: list[str] | None = None) -> int:
    """Console-script entrypoint registered in pyproject.toml as `eegfm`."""
