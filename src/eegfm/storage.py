"""Filesystem + S3 paths for eegfm artifacts.

Layout (canonical):

    Local NVMe (ephemeral, fast working set on the GPU box):
        $EEG_DATA_ROOT/                 — root, default /opt/dlami/nvme/eeg
        $EEG_DATA_ROOT/raw/hbn/         — raw HBN .set/.fdt (downloaded once)
        $EEG_DATA_ROOT/raw/tuab/        — raw TUAB v3.0.1 EDFs (rsync from NEDC)
        $EEG_DATA_ROOT/raw/tuev/        — raw TUEV v2.0.1 EDFs + .rec annotations
        $EEG_DATA_ROOT/derived/
        ...
        $EEG_DATA_ROOT/runs/<exp>/<id>/ — checkpoints + metrics during training
        $EEG_DATA_ROOT/models/          — HF cache (REVE / TFM / etc., later)
    ...
    S3 warehouse (persistent, cross-cloud):
        ...
        s3://eegmodel-warehouse/runs/eegfm/<NN_name>/<run_id>/    (synced at job end)
...
class Storage:
    ...
    ``$EEG_DATA_ROOT`` and falls back to ``DEFAULT_DATA_ROOT``. Pass an
    ...
    def raw_tuab(self) -> Path:
        """Local mirror of NEDC's TUAB v3.0.1 tree (after `eegfm tuh-rsync tuab`)."""
        return self.raw_root / "tuab"

    @property
    def raw_tuev(self) -> Path:
        """Local mirror of NEDC's TUEV v2.0.1 tree (after `eegfm tuh-rsync tuev`)."""
    ...
    def s3_run(self, experiment: str, run_id: str) -> str:
        return self.s3_uri("runs", "eegfm", experiment, run_id)
...
def from_env(env_var: str = "EEG_DATA_ROOT", default: str = DEFAULT_DATA_ROOT) -> Storage:
    """Construct a Storage rooted at $EEG_DATA_ROOT (or default).
