...
debugging (`python -m eegfm.sanity check_a`) or as a suite. All produce
...
def write_results_md(results: list[CheckResult], path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("# eegfm / mini-experiment 01 — Sanity baselines: results")
    lines.append("")
    lines.append(f"_Generated by `eegfm.sanity` at {time.strftime('%Y-%m-%dT%H:%M:%S UTC', time.gmtime())}_")
    lines.append("")
