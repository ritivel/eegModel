...
#
# Run from the GPU box (or any box with the eegfm venv installed):
#
#   SCRIPT=/home/ubuntu/eegModel/experiments/scripts/full_hbn_pipeline.sh
...
#
# Resumability: each `eegfm download` skips already-present .set, each
# `eegfm preprocess` skips already-present parquet, `sync-derived-up`
...
# Activate the eegfm venv so `eegfm` is on PATH inside subshells
